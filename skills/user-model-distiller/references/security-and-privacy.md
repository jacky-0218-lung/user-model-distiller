...
- Keep standard variation selectors (U+FE00-U+FE0F): emoji and CJK compatibility sequences need them, and legitimate use places at most one selector after a base character. Runs of two or more never occur in real text and can each carry four bits of hidden data, so `privacy_guard.py` raises the `variation_selector_run` warning, which blocks in `external-review` mode.
