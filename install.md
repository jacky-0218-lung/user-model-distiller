...
A digest proves only that the approved bytes did not change afterwards; it cannot prove that the reviewer saw all of them. Both `receipt` and `verify` therefore refuse a bundle whose **file names or file contents** contain any character in the class defined at the top of `tools/skill_bundle.py`: Unicode Tag characters, bidi embedding, override and isolate controls, zero-width spaces, word joiners, interlinear annotation controls, and soft hyphens. That class is not a complete inventory of every possible hidden character; it is the set this project refuses, kept identical across the normalizer, the privacy gate, the repository guard, and this tool. Reviewing the staged source remains necessary.
