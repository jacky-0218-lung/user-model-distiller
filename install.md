...
A digest proves only that the approved bytes did not change afterwards; it cannot prove that the reviewer saw all of them. Both `receipt` and `verify` therefore refuse a bundle whose **file names or file contents** contain any character in the class defined at the top of `tools/skill_bundle.py`: Unicode Tag characters, the variation selector supplement block (U+E0100-U+E01EF), bidi embedding, override and isolate controls, zero-width spaces, word joiners, interlinear annotation controls, and soft hyphens. That class is not a complete inventory of every possible hidden character; it is the set this project refuses, kept identical across the normalizer, the privacy gate, the repository guard, and this tool. Standard variation selectors (U+FE00-U+FE0F) remain allowed here because emoji and CJK compatibility sequences need them. Reviewing the staged source remains necessary.

Hashing and scanning run in one pass over the same read, so the scan result always describes the bytes the digest covers. A bundle approved under an earlier version that carries such a character now fails `verify` even when its digest matches; treat that as a finding to review, not as a tool error. Orthographic joiners (U+200C, U+200D) are preserved, because they are required by Persian, Arabic, Indic scripts, and emoji sequences. A byte-order mark at the very start of a file is allowed; the same character anywhere else is refused. Non-UTF-8 files are counted separately and excluded from the text scan.
...
3. Show the user an **Approval receipt** containing:
   ...
   - the `invisible_character_scan` result, which asserts that every UTF-8 file in
     the bundle was scanned against the refused character class above and that no
     file name or file content matched it; and
